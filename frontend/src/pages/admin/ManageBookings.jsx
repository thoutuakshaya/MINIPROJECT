...
function ManageBookings() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const res = await axios.get("https://https://miniproject-1asj.onrender.com/api/bookings/admin/all", {
        headers: {
  ...
  const handleDelete = async () => {
    try {
      const token = localStorage.getItem("userToken");
      await axios.delete(`https://https://miniproject-1asj.onrender.com/api/bookings/${confirmModal.bookingId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

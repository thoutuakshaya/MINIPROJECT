...
function AdminManageEvents() {
  ...
  const fetchEvents = async () => {
    try {
      setLoading(true);
      const res = await axios.get("https://miniproject-1asj.onrender.com/api/events", {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`https://miniproject-1asj.onrender.com/api/events/${confirmModal.eventId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });

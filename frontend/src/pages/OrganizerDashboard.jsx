...
function OrganizerDashboard() {
  ...
  const handleDelete = async (bookingId) => {
    ...
    try {
      await axios.delete(`https://https://miniproject-1asj.onrender.com/api/bookings/${bookingId}`, { headers });
  ...
  const fetchDashboardData = async () => {
    try {
      // Bookings
      const bookingsRes = await axios.get("https://https://miniproject-1asj.onrender.com/api/bookings/organizer", { headers });
      setBookings(bookingsRes.data);
      setPendingBookings(bookingsRes.data.filter(b => b.status === "pending").length);
      setexpertsBookedCount(bookingsRes.data.length);

      // Events
      const eventsRes = await axios.get("https://https://miniproject-1asj.onrender.com/api/events/myevents", { headers });
      setEventCount(eventsRes.data.length);

    
      // Profile Completion
      const profileRes = await axios.get("https://https://miniproject-1asj.onrender.com/api/users/profile", { headers });

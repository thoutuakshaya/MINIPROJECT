...
function ExploreEvents() {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        const res = await axios.get("https://https://miniproject-1asj.onrender.com/api/events/all", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setEvents(res.data);
      } catch (err) {
    ...
    const fetchJoinedEvents = async (userId) => {
      try {
        const res = await axios.get("https://https://miniproject-1asj.onrender.com/api/registrations/myevent", {
        params: { userId },
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleJoin = async (eventId) => {
    ...
    try {
      await axios.post(
        "https://https://miniproject-1asj.onrender.com/api/registrations/register",
        {
          eventId,
          name: user.name,
          email: user.email,
